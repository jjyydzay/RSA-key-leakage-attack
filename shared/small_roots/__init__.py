...
from sage.all import next_prime, GF, crt
DEBUG_ROOTS = None

# def reduce_lattice(L, delta=0.8, ifFlatter=False):
#     """
...
#     """
#     if ifFlatter:
#         print("Reducing a ",L.nrows()," x ",L.ncols(),"lattice with flatter...")
#         resultMat=flatter(L)
#     else:
#         print("Reducing a ",L.nrows()," x ",L.ncols(),"lattice with LLL...")
...
def reduce_lattice(B, delta=0.8):
    ...
    # Reduce basis
    start = time.time()  # Can't use cputime here, because it does not catch os.system(...).  
    print(f"Reducing a {B.nrows()} x {B.ncols()} lattice...")
...
def find_roots_julian(pr, polynomials, bounds):
    #Extract solutions
    ...
    for poly in polynomials:
        logging.debug(poly)
    while gbModulus < bounds:
        p = next_prime(p+1)

        pr = pr.change_ring(GF(p))
        I = pr*polynomials
        ...
        if success and len(solutions) == 1:
            ...
            if gbFailCounter > 100:
                print("Coppersmith heuristic failed. Could not extract solution from Gröbner basis.")
                return None
    ...
    yield {pr.gens()[ii]:solutions[ii] for ii in range(pr.ngens())}
...
def find_roots(pr, polynomials, bounds=2**512, method="groebner"):
    """
    Returns a generator generating all roots of a polynomial in some unknowns.
    The method used depends on the method parameter.
    :param pr: the polynomial ring
    ...
    """
    if pr.ngens() == 1:
        # print("Using univariate polynomial to find roots...")
        for polynomial in polynomials:
            yield from find_roots_univariate(pr.gen(), polynomial)
    else:
        # Always try this method because it can find roots the others can't.
        ...
        if method == "groebner":
            print("Using Groebner basis method to find roots...")
            yield from find_roots_groebner(pr, polynomials)
        elif method == "resultants":
            print("Using resultants method to find roots...")
            yield from find_roots_resultants(pr.gens(), polynomials)
        elif method == "variety":
            print("Using variety method to find roots...")
            yield from find_roots_variety(pr, polynomials)
        elif method == "julian":
            print("Using Julian's method to find roots...")
            yield from find_roots_julian(pr, polynomials, bounds)
